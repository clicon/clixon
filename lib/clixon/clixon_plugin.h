 ...
 * Macros
 */
#define upgrade_callback_register(h, cb, ns, from, rev, arg) upgrade_callback_reg_fn((h), (cb), #cb, (ns), (from), (rev), (arg))
...
int upgrade_callback_reg_fn(clicon_handle h, clicon_upgrade_cb cb, const char *strfn, char *ns, uint32_t from, uint32_t to, void *arg);
int upgrade_callback_delete_all(clicon_handle h);
int upgrade_callback_call(clicon_handle h, cxobj *xt, char *ns, uint32_t from, uint32_t to, cbuf *cbret);
