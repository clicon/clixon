 ...
 *
 * Recursively construct api-path to the top.
 * api-path is "URI-encoded path expression" definition in RFC8040 3.5.3
 * Example:
 *   yang:  container a -> list b -> key c -> leaf d
 *   api_path: /a/b=%s/d
               ...
               cxobj    **xerr)
{
    ...
    /* Special case: "//" is not handled properly by uri_str2cvec
...
/*! Construct an api_path from an XML node (single node not recursive)
 *
 * Add prefix if node is top-level, directly under (sub)module
 * @param[in]  x     XML node (need to be yang populated)
 * @param[out] cb    api_path, must be created before call
 * @retval     0     OK
 * @retval    -1     Error
 * @see xml2xpath   XPath instead of api-path
 * @note termination (spec vs flag) is different in xml2xpath
 */
int
xml2api_path_one(cxobj *x,
                 cbuf  *cb)
{
    int           retval = -1;
    yang_stmt    *y = NULL;
...
/*! Given an XML node, build an api-path recursively to top of tree
 *
 * Recurse up to parent until:
 * 1) no parent or
 * 2) xml flag is set or
 * 3) yang is anydata/anyxml
 * @param[in]  x     XML node (need to be yang populated)
 * @param[in]  flag  Recurse up until no parent or this xml flag is set
 * @param[out] cb    api_path, must be initialized
 * @retval     0     OK
 * @retval    -1     Error
 * @see xml2xpath   XPath instead of api-path (note termination is different)
 */
int
xml2api_path(cxobj   *x,
             uint16_t flag,
             cbuf    *cb)
{
    int        retval = -1;
