...
clicon_hash_t
clicon_hash_lookup_ptr(clicon_hash_t *hash,
                       void          *key)
{
    uint32_t      bkt;
    clicon_hash_t h;
...
void *
clicon_hash_ptr_value(clicon_hash_t *hash,
                      void          *key)
{
    clicon_hash_t h;
    ...
    h = clicon_hash_lookup_ptr(hash, key);
    if (h == NULL)
        return NULL; /* OK, key not found */
...
clicon_hash_t
clicon_hash_add_ptr(clicon_hash_t *hash,
                    void          *key,
                    void          *val)
{
    clicon_hash_t h;
    ...
    h->h_val = val;
    h->h_vlen =  HASH_NO_PTR;

    /* Add to list only if new variable */
    if (new)
        INSQ(h, hash[hash_ptr(key)]);
...
int
clicon_hash_del_ptr(clicon_hash_t *hash,
                    void          *key)
{
    clicon_hash_t h;
    ...
    if (h == NULL)
        return -1;
    DELQ(h, hash[hash_ptr(key)], clicon_hash_t);
    free(h);
...
int
clicon_hash_dump_ptr(clicon_hash_t *hash,
                     FILE          *f)
{
    int    retval = -1;
    size_t i;
    ...
    if (clicon_hash_keys(hash, &keys, &klen) < 0)
        goto done;
    for(i = 0; i < klen; i++) {
        val = clicon_hash_ptr_value(hash, keys[i]);
        printf("%p =\t 0x%p\n", keys[i], val);
    }
