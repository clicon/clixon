                    ...
                    char   *prefix)
{
    ...
    if (xml2ns(x, xml_prefix(x), &ns0) < 0)
       goto done;
    if (ns0 && strcmp(ns0, _namespace) == 0)
	goto ok; /* Already has right namespace */ 
    /* Is namespace already declared? */
    if (xml2prefix(x, _namespace, &prefix0) == 1){
	...
	/* Clear old prefix if any */
	   ...
	   xp = xml_parent(x);
       if (add_namespace(x, xp, prefix, _namespace) < 0)
	   goto done;	   
		 ...
		 char  *prefix0)
{
    ...
    if (xml2prefix(x1p, _namespace, &pexist) == 1){
	...
	if (xml2prefix(x1, _namespace, &pexist) == 1){
		...
		*/
		...
		if (yang_find_prefix_by_namespace(y, _namespace, &ptmp) < 0)
		    goto done;
	...
	if (add_namespace(x1, x1, prefix1, _namespace) < 0)
	    goto done;
	if (prefix1 && xml_prefix_set(x1, prefix1) < 0)
	    goto done;	    
    }
