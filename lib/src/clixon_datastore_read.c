               ...
               cxobj       **xerr)
{
    ...
    if ((ret = xmldb_get_cache(h, db, &x0t, xerr)) < 0)
        goto done;
    if (ret == 0)
        goto fail;
    ...
    if (clicon_option_bool(h, "CLICON_NACM_DISABLED_ON_EMPTY")){
        if (disable_nacm_on_empty(x1t, yspec0) < 0)
            goto done;
    }
           ...
           cxobj           **xerr)
{
    ...
    if ((ret = xmldb_get_copy(h, db, nsc, xpath, &x, xerr)) < 0)
        goto done;
    if (ret == 0)
        goto fail;
