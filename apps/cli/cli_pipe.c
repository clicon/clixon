 ...
 * @retval    -1     Error
 * @code
 *    format("Format by script") <script:string>("Name of python script"),
 *           pipe_script_format("script");
 * @endcode
 */
int
pipe_script_format(clixon_handle h,
                   cvec         *cvv,
                   cvec         *argv)
{
    int     retval = -1;
    ...
    if (script == NULL || strlen(script) == 0)
        goto ok;
    if ((dir = clicon_option_str(h, "CLICON_CLI_SCRIPT_DIR")) == NULL){
        clixon_err(OE_UNIX, 0, "CLICON_CLI_SCRIPT_DIR not set");
        goto done;
    }
    if ((cb = cbuf_new()) == NULL){
        clixon_err(OE_UNIX, errno, "cbuf_new");
