...
# Security: a list key value containing a single quote must be safely encoded
# into the xpath predicate generated by api_path2xpath (RESTCONF GET), not break
# out of it (XPath injection). %27 is the encoded single quote.
new "restconf PUT list entry with single-quote in key"
expectpart "$(curl $CURLOPTS -X PUT -H "Content-Type: application/yang-data+json" $RCPROTO://localhost/restconf/data/list:c/a=a%27b,y2 -d '{"list:a":{"b":"a'\''b","c":"y2","nonkey":"0"}}')" 0 "HTTP/$HVER 201"

new "restconf GET list entry with single-quote in key (xpath injection safe)"
expectpart "$(curl $CURLOPTS -X GET -H "Accept: application/yang-data+json" $RCPROTO://localhost/restconf/data/list:c/a=a%27b,y2)" 0 "HTTP/$HVER 200" '{"list:a":\[{"b":"a'\''b","c":"y2","nonkey":"0"}\]}'

new "restconf DELETE single-quote key entry (cleanup so later list asserts are unaffected)"
expectpart "$(curl $CURLOPTS -X DELETE $RCPROTO://localhost/restconf/data/list:c/a=a%27b,y2)" 0 "HTTP/$HVER 204"

# Double quote in key: xpath_literal_encode wraps in single quotes
new "restconf PUT list entry with double-quote in key"
expectpart "$(curl $CURLOPTS -X PUT -H "Content-Type: application/yang-data+json" $RCPROTO://localhost/restconf/data/list:c/a=a%22b,y3 -d '{"list:a":{"b":"a\"b","c":"y3","nonkey":"0"}}')" 0 "HTTP/$HVER 201"

new "restconf GET list entry with double-quote in key (xpath literal safe)"
expectpart "$(curl $CURLOPTS -X GET -H "Accept: application/yang-data+json" $RCPROTO://localhost/restconf/data/list:c/a=a%22b,y3)" 0 "HTTP/$HVER 200" '{"list:a":\[{"b":"a\\"b","c":"y3","nonkey":"0"}\]}'

new "restconf DELETE double-quote key entry (cleanup)"
expectpart "$(curl $CURLOPTS -X DELETE $RCPROTO://localhost/restconf/data/list:c/a=a%22b,y3)" 0 "HTTP/$HVER 204"

# Both single and double quote: exercises xpath_literal_encode concat() branch
new "restconf PUT list entry with both single and double-quote in key"
expectpart "$(curl $CURLOPTS -X PUT -H "Content-Type: application/yang-data+json" $RCPROTO://localhost/restconf/data/list:c/a=a%27%22b,y4 -d '{"list:a":{"b":"a'\''\"b","c":"y4","nonkey":"0"}}')" 0 "HTTP/$HVER 201"
#expectpart "$(curl $CURLOPTS -X PUT -H "Content-Type: application/yang-data+json" $RCPROTO://localhost/restconf/data/list:c/a=a%27%22b,y4 -d '{"list:a":{"b":"a'\''\"b","c":"y4","nonkey":"0"}}')" 0 "HTTP/$HVER 201"

new "restconf GET list entry with both quotes in key (xpath concat branch)"
expectpart "$(curl $CURLOPTS -X GET -H "Accept: application/yang-data+json" $RCPROTO://localhost/restconf/data/list:c/a=a%27%22b,y4)" 0 "HTTP/$HVER 200" '{"list:a":\[{"b":"a'\''\\"b","c":"y4","nonkey":"0"}\]}'

new "restconf DELETE both-quotes key entry (cleanup)"
expectpart "$(curl $CURLOPTS -X DELETE $RCPROTO://localhost/restconf/data/list:c/a=a%27%22b,y4)" 0 "HTTP/$HVER 204"
